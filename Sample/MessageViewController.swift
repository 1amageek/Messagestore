...
class MessageViewController: Message<User, Room, Transcript>.MessagesViewController {
    ...
    override func transcript(_ transcript: Document<Transcript>, willSendTo room: Document<Room>, with batch: Batch) {
        guard let text: String = self.textView.text else { return }
        if text.isEmpty { return }
        transcript.data?.text = text
        self.textView.text = nil
    }

    override func transcript(_ transcript: Document<Transcript>, didSend room: Document<Room>, reference: DocumentReference?, error: Error?) {
